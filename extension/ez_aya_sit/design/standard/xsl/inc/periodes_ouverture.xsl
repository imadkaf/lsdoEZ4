...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="periodes-ouverture">
		<xsl:if test="ouvertureAnnee/@value = '1'">
			<div>
				<xsl:attribute name="style">color:black;background:#CCCCCC url('<xsl:value-of select="$cheminImagesDesign"/>picto_calendrier.png') no-repeat 0 50%;padding:3px 3px 3px 23px;margin-bottom:5px</xsl:attribute>
			...
			<div style="background-color:#CCCCCC;padding:3px;margin-bottom:5px">
				...
				<div style="border-left:2px solid black;padding-left:3px">
					<xsl:for-each select="periodesOuvertures/periodeOuverture">
						<div>
							<xsl:attribute name="style">background:url('<xsl:value-of select="$cheminImagesDesign"/>picto_calendrier.png') no-repeat 0 50%;padding:2px 0 2px 23px</xsl:attribute>
