    ...
    function processViewActions( $objectAttribute, &$survey, &$validation )
    {
        ...
        if ( $continueViewActions === true )
        {
            ...
            if ( $actionContinue === true )
            {
                ...
                /** Gestion de reCaptcha (validation) **/ 
                $recaptchaValidation = recaptchaType::reCAPTCHAValidate($http);
                
		    	if(!$recaptchaValidation){
		    		$validation['error'] = true;
		    		$validation['errors'][] = array('message' => 'Le code de sécurité est incorrect', 'question_number' => 'Code de sécurité');
		    	}
                ...
                if ( $http->hasPostVariable( $postSurveyStoreButton ) && $validation['error'] == false )
                {
                    ...
                    if ( $http->hasPostVariable( $postReceiverID ) )
                    {
                        ...
                        /** Récupération du nom de l'internaute entré dans le formulaire **/
                        $userName = '';
	                    foreach($surveyList as $surveyAttribute){
	                    	
	                    	/** Recherche des champs email contenant l'attribut 'user_name' **/
							$userNameAttribute = $surveyAttribute->attribute('text3');
	                    	if($userNameAttribute == 'user_name'){
	                    		$userName = $surveyAttribute->attribute('answer');
	                    		break;
	                    	}						                    	
	                    }
	                    /** Définition de l'objet du mail **/
	                    $post = $http->attribute('post');
	                    $subject = $post['form_name'];
                        ...
                        /** Modification de l'objet du mail : voir plus haut **/
                    ...
                    /** Ajout de l'envoi de mail à  l'utilisateur **/
                    ...
                    foreach($surveyList as $surveyAttribute){
                    	...
                    	/** Recherche des champs de formulaire correspondant à  un champ mail **/
                    	if(get_class($surveyAttribute) == 'eZSurveyEmailEntry'){

                    		/** Recherche des champs email contenant l'attribut 'user_email' **/
                    		$userEmailAttribute = $surveyAttribute->attribute('text3');
                    		if($userEmailAttribute == 'user_email'){
                    			 
                    			/** Récupération du mail entré par l'utilisateur **/
                    			$mailTo = $surveyAttribute->attribute('answer');
                    			...
                    			$tpl_email->setVariable( 'survey', $survey );
                    			$tpl_email->setVariable( 'survey_questions', $surveyList );
                    			$tpl_email->setVariable( 'mail_content', $post['mail_content']);

                    			$templateResult = $tpl_email->fetch( 'design:survey/user_form.tpl' );
                    			
                    			/** Sujet du mail **/
                    			$subject = $post['mail_subject'];
                    			if(!$subject){
									$ini = eZINI::instance( "cg41.ini" );
									$subject = $ini->variable( "EzsurveyMails", "defaultSubject" );
	                    			//$subject = $tpl_email->variable( 'subject' );
                    			}
