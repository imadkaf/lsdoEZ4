<label class="lsdo-label non-textarea">{$question.text|wash('xhtml')} {switch match=$question.num}{case match=1}<strong class="required">*</strong>{/case}{case match=2}<strong class="required">*</strong>{/case}{/switch}</label>
...
{section-else}
	...
	{switch match=$question.num}
		...
		{section var=option loop=$options}
		    <div class="element radio-ligne"><label><input id="{$attr_id}_{$option.id}" name="{$attr_name}" type="radio" value="{$option.value}"{section show=$option.toggled|eq(1)} checked="checked"{/section} onclick="synchFormElements( '{$attr_id}_{$option.id}', '{$attr_alternative}', false );" /><span>{$option.label}</span></label></div>
		{/section}
		{if $question.extra_info.enabled|eq(1)}
		...
		<div class="block float-break">
		{section var=option loop=$options}
		    <div class="block checkbox-column"><label><input name="{$attr_name}[]" type="checkbox" value="{$option.value}"{section show=$option.toggled|eq(1)} checked="checked"{/section} /><span>{$option.label}</span></label></div>
		{/section}
		{if $question.extra_info.enabled|eq(1)}
