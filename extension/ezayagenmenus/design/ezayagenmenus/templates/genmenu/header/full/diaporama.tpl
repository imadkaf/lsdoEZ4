	...
	<script type="text/javascript">	
		...
		{foreach $diapos as $diapo}
			{if $diapo.data_map.url_interne.has_content}
				{set $url = concat($hostname, $diapo.data_map.url_interne.content.main_node.url_alias|ezurl('no'))}
			{else}
				{set $url = $diapo.data_map.url_externe.content}
			{/if}
			{ldelim}
					"title" : "{$diapo.data_map.titre.content}",
					"image" : "{concat($hostname, $diapo.data_map.image.content.original.full_path|ezurl('no'))}",
					"url" : "{$url}",
