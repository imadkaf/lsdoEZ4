...
Input:
 ...
 img_class     - Optional css class to wrap in the <img> tag, the
                 class will be placed in a <img> tag.
 dimension	   - Define if the <width> and <height> tags will placed in the <img> tag
...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        {if $href}<a href={$href}{if and( is_set( $link_class ), $link_class )} class="{$link_class}"{/if}{if and( is_set( $link_id ), $link_id )} id="{$link_id}"{/if}{if $target} target="{$target}"{/if}{if and( is_set( $link_title ), $link_title )} id="{$link_title|wash}"{/if}>{/if}
            <img{if $img_class} class="{$img_class}"{/if} src="{concat($hostNamePrefix,$image.url|ezroot('no'))}"{if $dimension} width="{$image.width}" height="{$image.height}"{/if} {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
