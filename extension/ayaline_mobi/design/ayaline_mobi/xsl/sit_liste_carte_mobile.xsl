						...
						<![CDATA[
							...
							var contentString = '<div class="moninfobulle">'+
									...
									']]><xsl:if test="string-length(commentaires/commentaire1) &gt; 0">
											...
											<p class="commentaire">
												<xsl:call-template name="string-replace-all"><xsl:with-param name="text" select="$commentaire1NettoyeCoupe"/><xsl:with-param name="replace" select="$apos"/><xsl:with-param name="by" select="'&amp;apos;'"/></xsl:call-template><xsl:if test="string-length($commentaire1Nettoye) &gt; 100">&amp;hellip;</xsl:if>
											</p>
