...
{* Création d'un tableau JS avec toutes les valeurs (id ez), et leur code postal *}
{def $communeClassIdentifier = $attribute.content.class_identifier}
{def $communes = fetch('content', 'tree', hash( 'parent_node_id', ezini('NodeSettings','MediaRootNode', 'content.ini') ,
												'class_filter_type', 'include',
												'class_filter_array', array('commune') ))}
<script type="text/javascript">
	var cpCommunes = new Array();
	{foreach $communes as $commune}
		cpCommunes[{$commune.contentobject_id}] = '{$commune.data_map.code_postal.content}';
	{/foreach}	
</script>											
