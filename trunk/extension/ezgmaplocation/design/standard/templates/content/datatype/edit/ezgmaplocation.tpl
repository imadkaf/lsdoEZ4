{* Récupérer l'attribut lieu *}
{def $objAttributes = $attribute.object.data_map}
{def $adresseAttributeId = false()}
{def $communeAttributeId = false()}
{def $identifiantAddresse = 'adresse'}
{def $identifiantCommune = 'commune'}
{if is_set($objAttributes[$identifiantAddresse])}
	{set $adresseAttributeId = $objAttributes[$identifiantAddresse]['contentclassattribute_id']}
{/if}
{if is_set($objAttributes[$identifiantCommune])}
	{set $communeAttributeId = $objAttributes[$identifiantCommune]['contentclassattribute_id']}
{/if}
...
//création des identifiants des inputs à mettre à jour
var ezAttributeAdressIdentifier = "ezcoa-{$adresseAttributeId}_{$identifiantAddresse}";
var ezAttributeCommuneIdentifier = "ezcoa-{$communeAttributeId}_{$identifiantCommune}";
var ezAttributeGmapsAdressIdentifier = "ezgml-address-{$attribute.id}";
var ezAttributeGmapsLatitudeIdentifier = "ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_latitude";
var ezAttributeGmapsLongitudeIdentifier = "ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_longitude";
...
function eZGmapLocation_MapControl( attributeId, latLongAttributeBase )
{
    ...
    //pré remplir le champ adresse de gmaps, si on a un lieu et rien d'autre
    if( lieux === undefined ) {
    	var lieux = '';
    }
	if( $('#'+ezAttributeAdressIdentifier).val() == '' && $('#'+ezAttributeGmapsAdressIdentifier).val() == '' ) {
		$('#'+ezAttributeGmapsAdressIdentifier).val(lieux);
	}
	...
	/**
	* Adaptation de l'affichage de l'adresse pour prendre en compte notre champ adresse.
	*/
    function updateAndShowAdress() {
    	//mettre à jour uniquement si l'on a pas déja des coordonnées
    	//if( $('#'+ezAttributeGmapsLongitudeIdentifier).val() != '' || $('#'+ezAttributeGmapsLongitudeIdentifier).val() != '' ) {
    	//	return;
    	//}
    	
    	//récupérer la valeur du champ adresse + CP + Commune
    	var addr = $('#'+ezAttributeAdressIdentifier).val();
    	if( addr != '' ) {
	    	if( $('#'+ezAttributeCommuneIdentifier).val() != '' ) {
	        	//cp + commune si on a choisi qqch.
	    		addr = addr + ' '+ cpCommunes[$('#'+ezAttributeCommuneIdentifier).val()] +' '+$('#'+ezAttributeCommuneIdentifier+' option:selected').text();
	    	}
    	}

    	//mettre à jour le champ adresse de gmaps, en supprimant les retours à la ligne
    	$('#'+ezAttributeGmapsAdressIdentifier).val(addr.replace(/(\r)?\n/gm, ' '));
    	
    	alert('Adresse modifiée : pensez à vérifier la carte.');
    ...
    if (GBrowserIsCompatible())
    {
		...
		//ajouter l'evenement sur le lieu
 		document.getElementById( ezAttributeAdressIdentifier ).onblur = updateAndShowAdress;
 		document.getElementById( ezAttributeCommuneIdentifier ).onchange = updateAndShowAdress;
