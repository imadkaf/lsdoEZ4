...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img{if $img_class} class="{$img_class}"{/if} src={$image.url|ezroot}{if $dimension} width="{$image.width}" height="{$image.height}"{/if} {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
