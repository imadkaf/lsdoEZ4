			...
			<ul class="list-inline-bis">
				<xsl:for-each select="resultats/details/detail">
					...
					<li>
						...
						<xsl:if test="count(criteres/critere[count(modalites/modalite[contains($criteresAffiches, concat('|', @id, '|')) or contains($criteresAffiches, concat('|', ../../@id, '|'))]) &gt; 0 and @id != '851000011' and @id != '851000028' and @id != '851000308' and @id != '851000033' and @id != $critere-apartirde]) &gt; 0">
							<ul class="criteres liste-hotel">
								<xsl:for-each select="criteres/critere[count(modalites/modalite[contains($criteresAffiches, concat('|', @id, '|')) or contains($criteresAffiches, concat('|', ../../@id, '|'))]) &gt; 0 and @id != '851000011' and @id != '851000028' and @id != '851000308' and @id != '851000033' and @id != $critere-apartirde]">
