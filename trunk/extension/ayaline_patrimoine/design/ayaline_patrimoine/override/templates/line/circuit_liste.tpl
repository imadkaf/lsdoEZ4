{if $simple}
 ...
 <article>
		...
		<a href={$node.url_alias|ezurl} class="image-lien ui-link" rel='external'>
			{attribute_view_gui attribute=$node.data_map.declinaison_circuit.content.current.data_map.picto_pieton image_class='original' alt_text=supp_br($node.name) title=supp_br($node.name)}
		</a>
</article>
