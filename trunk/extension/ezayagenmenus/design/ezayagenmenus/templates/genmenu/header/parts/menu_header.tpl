...
<div class="menu" style="position: absolute; bottom: 15px;top:auto;right: 0;">
    <ul>
        ...
        {foreach $rNode.data_map.main_menu.content.main_node.children as $key => $sMenu sequence array( 'bottom', 'bottom-2', '' ) as $menuStyle}
            {if and(is_set($view_parameters[$extAyaWidgetFilterName]),$extAyaWidgetHiddenHeaderMenuItems|contains($sMenu.node_id))|not}
                ...
                <li class="rubrique{$key}"> {* en principe rubrique0 => Découvrir, rubrique1 => Séjourner, rubrique2 => A la une *}
                    ...
                    <a class="element" href="#"><span>{$sMenu.data_map.content.content.name|wash}</span></a>
                    {*
                                ...
                                <ul class="top">
                                    {foreach $topicsList.children as $topic} {* themes Mer/ville/plage/Nature définis dans le pagelayout *}
                                            {if ne($topic.node_id, ezini('NodeSettings','TopicDefaut','content.ini'))} {* on n'affiche pas le thème par défaut *}
                                                    {node_view_gui content_node=$topic view=line topicIds=$topicIds redirectURI=concat($hostname, $module_result.uri|ezurl('no'))}
                                                {/if}
                                                {/foreach}
                                                </ul>
                                                <div class="clear"></div>
