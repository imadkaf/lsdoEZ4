...
if (isset($_GET["idc"])) {
    ...
    if ($xmlResultat) {
        ...
        foreach ($xmlResultat->details->detail as $detail) {
            ...
            if ( ( (isset($_GET["idp"]) && $_GET["idp"] == $idProduit) || !isset($_GET["idp"]) ) && ( count($testGeoLat) > 0 && count($testGeoLng) > 0 && is_numeric((string) $testGeoLat[0]) && is_numeric((string) $testGeoLng[0]) ) && ($typeAffichage == 1 || ($typeAffichage == 2 and $dateOuvertureIsOk)) ) {
