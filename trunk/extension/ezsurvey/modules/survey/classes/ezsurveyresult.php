...
class eZSurveyResult extends eZPersistentObject
{
    ...
    */
    ...
    static function exportCSV( $contentObjectID, $contentClassAttributeID, $languageCode )
    {
		...
		/** Récupération des questions de type MultipleChoice **/
        $questionsMultiples = array();
        foreach($questionList as $question){
        	if($question->Type == 'MultipleChoice'){
	        	$questionsMultiples[$question->ID] = $question; 
        	}
        }
        ...
        foreach( array_keys( $rows ) as $key )
        {
        	...
        	/** Pour toutes les questions de type 'MultipleChoice', on modifie la valeur pour l'export : récupération du label **/
            if(array_key_exists($row['question_id'], $questionsMultiples)){
            	/** Récupération de la question multiple **/
	            $question = $questionsMultiples[$row['question_id']];
	            /** Récupération du label en fonction de la valeur de l'élément de formulaire **/
	        	$options = $question->Options;
	            foreach($options as $option){
	            	if($option['id'] == $row['text']){
